...
static class MyLocalCacheGetWorldInfoFromCampaignPatch
{
    static bool Prefix(
        ...
        ref List<string> foldersToSearchForCampaigns,
        ref List<MyWorldInfo> __result
    )
    {
        List<MyWorldInfo> list = new();
        MyObjectBuilder_CampaignSM myObjectBuilder_CampaignSM =
            MyPlatformGameSettings.CONSOLE_COMPATIBLE
                ? campaign.GetCrossPlatformStateMachine()
                : campaign.GetStateMachine();
        if (myObjectBuilder_CampaignSM != null)
        {
            MyObjectBuilder_CampaignSMNode myObjectBuilder_CampaignSMNode =
                MyCampaignManager.Static.FindStartingState(myObjectBuilder_CampaignSM);
            if (myObjectBuilder_CampaignSMNode != null)
            {
                string saveFileDirectory = Path.GetDirectoryName(
                    PathCache.ResolveAbsolute(myObjectBuilder_CampaignSMNode.SaveFilePath)
                );
                string fullName = Directory
                    .GetParent(Path.Combine(MyFileSystem.ContentPath, saveFileDirectory))
                    .FullName;
                if (
                    (campaign.IsLocalMod || !campaign.IsVanilla)
                    && !string.IsNullOrEmpty(campaign.ModFolderPath)
                    && !Directory.Exists(fullName)
                )
                {
                    fullName = Directory
                        .GetParent(Path.Combine(campaign.ModFolderPath, saveFileDirectory))
                        .FullName;
                }
...
{
    static bool Prefix(
        MyGuiScreenNewGameScenarioSelection __instance,
        MyObjectBuilder_Campaign cItem,
        ref List<string> foldersToSearchForCampaigns
    )
    {
        List<MyWorldInfo> worldInfoFromCampaign = MyLocalCache.GetWorldInfoFromCampaign(
            cItem,
            ref foldersToSearchForCampaigns
        );
        MyObjectBuilder_CampaignSM myObjectBuilder_CampaignSM =
            MyPlatformGameSettings.CONSOLE_COMPATIBLE
                ? cItem.GetCrossPlatformStateMachine()
                : cItem.GetStateMachine();
        if (myObjectBuilder_CampaignSM != null)
        {
            MyObjectBuilder_CampaignSMNode myObjectBuilder_CampaignSMNode =
                MyCampaignManager.Static.FindStartingState(myObjectBuilder_CampaignSM);
            if (myObjectBuilder_CampaignSMNode != null)
            {
                ...
                if (
                    (cItem.IsLocalMod || !cItem.IsVanilla)
                    && !string.IsNullOrEmpty(cItem.ModFolderPath)

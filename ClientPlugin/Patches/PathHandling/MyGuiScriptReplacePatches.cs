...
static class MyGuiIngameScriptsPageReplaceFromEditorPatch
{
    static bool Prefix(MyGuiIngameScriptsPage __instance)
    {
        if (
            ...
            || __instance.GetCodeFromEditor == null
            || !Directory.Exists(__instance.m_localScriptFolder)
        )
            return false;
        ...
        MyGuiSandbox.AddScreen(
            MyGuiSandbox.CreateMessageBox(
                ...
                messageCaption: MyTexts.Get(
                    MySpaceTexts.ProgrammableBlock_ReplaceScriptNameDialogTitle
                ),
...
static class MyGuiBlueprintScreenReplaceScriptPatch
{
    static bool Prefix(MyGuiBlueprintScreen_Reworked __instance)
    {
        ...
        if (
            __instance.SelectedBlueprint.Type == MyBlueprintTypeEnum.CLOUD
            || __instance.SelectedBlueprint.Type == MyBlueprintTypeEnum.WORKSHOP
            || (
                __instance.SelectedBlueprint.Type == MyBlueprintTypeEnum.LOCAL
                && MySandboxGame.Config.EnableCloud
            )
        )
        {
            return true;
            ...
            MyGuiSandbox.CreateMessageBox(
                ...
                callback: callbackReturn =>
                {
                    ...
                    if (File.Exists(scriptPath))
                        File.WriteAllText(
                            scriptPath,
                            __instance.m_getCodeFromEditor(),
                            Encoding.UTF8
                        );
                }
            )
